...
if($env:bolt_dev -eq "true"){
    $global:boltDev = $true
}
elseif($env:bolt_dev -eq "false"){
...
#     Throw "Powershell 7 is required. (https://www.thomasmaurer.ch/2019/07/how-to-install-and-update-powershell-7/)"
# }
...
$Versions = Get-CurrentDeploymentVersions -Versions $Config.publish.releases -Branch $Branch -DefaultBranch $Config.publish.defaultRelease
...
# if (!$config.registry) {
#     throw "remote registry not defined in config"
# }

# #todo add a check for .gitignore and that it have a .biceptemp
$TempFolder = join-path (Get-GitRoot) ".bicepTemp"
...
# Update-TypeData -MemberType NoteProperty -TypeName System.IO.FileInfo -MemberName 'RepoName' -Value '' -Force -
$BicepModules = Get-ChildItem -Path $ModuleRoot -Filter "main.bicep" -Recurse|%{
...
# if ($Name) {
#     $BicepModules = $BicepModules | Where-Object {
#         [System.io.path]::GetRelativePath($ModuleRoot, $_.Directory.FullName).Replace("\", "/") -like $name
#     }
# }
...
# return
# $ThrottleLimit = 2
# if($BicepModules.count -gt 1)
# {
#     $throttleLimit = $BicepModules.count / 2
# }
...
$job = $ModulesToPublish| ForEach-Object -ThrottleLimit $ModulesToPublish.count -AsJob -Parallel {
        ...
        :versionUpload foreach ($activeVersion in $($using:Versions)) {
            ...
            switch ($activeVersion.trigger) {
                ...
                "semantic" {
                    ...
                    $VersionDefinition = Get-Content $BicepFile.FullName | select -First 10 | ? { $_ -like "//module_version*" } | % {
                        $_
                    ...
                    if ($AcrRepositoryTag -and $AcrRepositoryTag.name -ge $(($UsingVersion | ? { $_ }) -join '')) {
                        # Write-BoltLog "remote repo tag: $($AcrRepositoryTag.Name)"
                        #get the layer
                        $AcrRepositoryTagLayer = $null
                        try {
                        ...
                        # Write-BoltLog ($AcrRepositoryTagLayer|ConvertTo-Json -depth 10)
    
                        # if (Test-ShouldSkipUpdateModule -Layer $AcrRepositoryTagLayer -LocalTemplate $armFile) {
                        #     $thisStat.skipped++
                        #     Write-BoltLog "Skipped! $($activeVersion.name)/$($activeVersion.trigger) -> $($activeVersion.value) is already uploaded ($($AcrRepositoryTagLayer[0].tag))" -level warning
                        #     continue :versionUpload
                        # }

                    
                        #extract just regex keys
                        $versionMap = @{
                        ...
                        if ($remoteVersion -ge $UsingVersion[1]) {
                            ...
                            # $Changes.Values| Write-ModuleUpdateStatus

                            # Write-BoltLog ($Changes|ConvertTo-Json -depth 10)
                            ...
                            # else {
                            #     $thisStat.skipped++
                            #     Write-BoltLog "Skipped! something happened: should generate new schemantic version using remote version as base, but none of the versjon checks returned true." -level warning
                            #     continue :versionUpload
                            # }
...
try {
    ...
    while ($job.state -eq 'Running') {
        ...
        $states = $job.ChildJobs | group state | % { $_.name + ":" + $_.count }

        #calculate average time for completed childjobs
        $CompletedAverageSeconds = ($out.Values | ? { $_.state -eq 'completed' } | % { $_.timer.elapsed.totalseconds } | measure -Average).Average
        ...
        $job.ChildJobs | ? { $_.State -eq 'completed' -and !$out.($_.id).processed } | % {
    ...
    $job.ChildJobs | ? { !$out[$_.id].processed } | % {
        $out.($_.id).processed = $true
        $_ | receive-job
    }
} catch {
...
if($global:publishAction.CleanRegistry){
    ...
    Foreach($repo in $Repos|?{$_.ImageName -notin $BicepModules.RepoName}){
...
$statistics.values.keys | Select-Object -Unique | % {
